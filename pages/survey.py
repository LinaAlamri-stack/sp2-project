...
if submitted:
    ...
    # التحقق اذا فيه سؤال ما له اجابه
    if any(answer is None for answer in questions):
        ...
        #  انشاء قاموس يحتوي البيانات النهائية بنفس أسماء أعمدة الداتا
        user_input = {
            ...
            "low_energy_frequency": low_energy_map[q9_low_energy]
               ...
               # حفظ بيانات اليوزر في session_state عشان الداشبورد يقرأها
        st.session_state["caffeine_per_day"] = user_input["caffeine_per_day"]
        st.session_state["fast_food_per_week"] = user_input["fast_food_per_week"]
        st.session_state["sleep_hours"] = user_input["sleep_hours"]
        st.session_state["physical_activity_days"] = user_input["physical_activity_days"]
        st.session_state["screen_hours"] = user_input["screen_hours"]
        st.session_state["sleep_quality"] = user_input["sleep_quality"]
        st.session_state["eat_after_10pm"] = user_input["eat_after_10pm"]
        st.session_state["caffeine_after_8pm"] = user_input["caffeine_after_8pm"]
        st.session_state["low_energy_frequency"] = user_input["low_energy_frequency"]
